...
let books = [
    ...
    { book_id: 'B002', book_name: 'To Kill a Mockingbird', author: 'Harper Lee',           genre: 'Fiction',     year: 1960, status: 0 },
    { book_id: 'B003', book_name: '1984',                  author: 'George Orwell',        genre: 'Dystopia',    year: 1949, status: 1 },
    ...
    for (let i = 0; i < books.length; i++) {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>${book.genre}</td>
                <td>${book.year}</td>
                <td>${book.status == 1 ? 'Available' : 'Borrowed'}</td>
            </tr>
        `;
    }
}
...
function returnBook() {
    ...
    if (book.status == 1) {
        document.getElementById('e-ret').innerHTML = 'This book is not currently borrowed';
        return;
    }
